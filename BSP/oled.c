...
#define OLED_CS(x)                                                                                                                          \
    do {                                                                                                                                    \
        x == 1 ? HAL_GPIO_WritePin(OLED_CS_Port, OLED_CS_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(OLED_CS_Port, OLED_CS_Pin, GPIO_PIN_RESET); \
    } while (0)

#define OLED_RST(x)                                                                                                                             \
    do {                                                                                                                                        \
        x == 1 ? HAL_GPIO_WritePin(OLED_RST_Port, OLED_RST_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(OLED_RST_Port, OLED_RST_Pin, GPIO_PIN_RESET); \
    } while (0)

#define OLED_DC(x)                                                                                                                          \
    do {                                                                                                                                    \
        x == 1 ? HAL_GPIO_WritePin(OLED_DC_Port, OLED_DC_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(OLED_DC_Port, OLED_DC_Pin, GPIO_PIN_RESET); \
    } while (0)

#define OLED_SCL(x)                                                                                                                             \
    do {                                                                                                                                        \
        x == 1 ? HAL_GPIO_WritePin(OLED_SCL_Port, OLED_SCL_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(OLED_SCL_Port, OLED_SCL_Pin, GPIO_PIN_RESET); \
    } while (0)

#define OLED_SDA(x)                                                                                                                             \
    do {                                                                                                                                        \
        x == 1 ? HAL_GPIO_WritePin(OLED_SDA_Port, OLED_SDA_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(OLED_SDA_Port, OLED_SDA_Pin, GPIO_PIN_RESET); \
    } while (0)
#endif
/*******************基础驱动函数**********************/
/* 如果使能硬件IIC */
 ...
 * @brief OLED初始化函数
 * @param None
 * @attention   如果FreeRTOS被使用，OLED初始化应在FreeRTOS内核启动前完成
 * @retval None
 */
void OLED_Init(void)
{
